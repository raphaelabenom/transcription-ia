import { Github, FileMusic, Upload, Wand2 } from 'lucide-react'
      ...
      <main className='flex-1 p-6 flex gap-6'>
        ...
        <aside className='w-80 space-y-6'>
          ...
          <form className='space-y-6'>
            <label
              htmlFor="audio"
              className='border flex rounded aspect-video cursor-pointer border-dashed text-sm flex-col gap-2	items-center justify-center text-muted-foreground hover:bg-primary/5 text-center'>
              <FileMusic className='w-4 h-4' />
              Carregar Audio
            </label>
            <input type="file" id="audio" accept='audio/mp3' className='sr-only' />

            <Separator />
            <div className='space-y-2'>

              <Label htmlFor='transcription_prompt'>Prompt de transcrição</Label>
              <Textarea id='transcription_prompt' className='h-20 leading-relaxed rounded resize-none' placeholder='Inclua palavras-chaves mencionadas no áudio separadas por vírgula (,)' />
            </div>
